...
class Requester:
    def __init__(self, config_=None,anticaptcha_key='',manual_captcha_decode=False,proxies=None):
        current_config = config_ if config_ else config
        self.anticaptcha_key = anticaptcha_key
        self.proxies = proxies
        self.manual_captcha_decode = manual_captcha_decode
    ...
    def _parse_response(self, response):
        ...
        if response.status_code == 201:
            ...
            if errorCode == '403004':
                ...
                # return False, {"err":"Captcha!"}
                from getcontact.decode_captcha import CaptchaDecode
                if self.manual_captcha_decode:
                    raise Exception('Captcha!')
                if self.anticaptcha_key == '':
                    c = CaptchaDecode()
                    code, path = c.decode_response(response)
                else:
                    c = CaptchaDecode(anticaptcha_key=self.anticaptcha_key)
                    code, path = c.decode_response_anticaptcha(response)
                print(code)
